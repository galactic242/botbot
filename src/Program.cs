    ...
    public static class Program
    {
        ...
        private static async Task HandleMessage(SocketMessage msg)
        {
            ...
            try
            {
                ...
                try
                {
                    result = new Deobfuscator().Deobfuscate(sourceCode);
                }
                catch (Exception ex)
                {
                    cts.Cancel();
                    await statusMsg.ModifyAsync(m => m.Content = "Error during conversion");
                    await msg.Channel.SendMessageAsync($"error {ex.Message}");
                    Busy.Remove(msg.Author.Id);
                    return;
                }

                cts.Cancel();
                await Task.WhenAny(spinner, Task.Delay(300));
                ...
                {
                    bw.Write(System.Text.Encoding.UTF8.GetBytes("-- bytecode by galactic join now\n"));
                    bw.Write(result);
                }
                ...
                {
                    await msg.Channel.SendFileAsync(fs, bytecodeFile, $"{msg.Author.Mention} here is your bytecode file");
                }
